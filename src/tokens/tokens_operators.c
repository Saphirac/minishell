...
/*   Updated: 2023/03/09 03:32:56 by mcourtoi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_get_double_op(t_shell *shell, int *i, int j)
{
	char	*ret;
...
char	*ft_get_operator(t_shell *shell, int *i, int j)
{
	...
	{
		ret = ft_strndup((shell->line + j), (*i - 1 - j));
		if (!ret)
			return (NULL);
	...
	if (shell->line[*i] == '|')
		...
		ret = ft_get_double_op(shell, i, j);
